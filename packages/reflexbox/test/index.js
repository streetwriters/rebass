...
describe("Box", () => {
  ...
  test("renders with color props", () => {
    const json = render(<Box color="tomato" bg="black" />);
  ...
  test("renders with flexbox props", () => {
    const json = render(<Box flex="1 1 auto" alignSelf="flex-start" />);
  ...
  test("removes style props", () => {
    const json = render(<Box color="blue" fontSize={2} width={1} />);
    expect(Object.keys(json.props)).toEqual(["className"]);
  });
...
describe("Flex", () => {
  test("renders with display flex", () => {
    const json = render(<Flex />);
    expect(json).toHaveStyleRule("display", "flex");
  });

  test("renders with Box props", () => {
    const json = render(<Flex color="tomato" />);
    expect(json).toHaveStyleRule("color", "tomato");
  });

  test("as prop does not break Box props", () => {
    const json = render(
      <Flex as="footer" width={1 / 2} fontSize={3} color="tomato" />
    );
